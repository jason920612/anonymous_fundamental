"""Phase 49 experiment: adaptive γ via signal entropy.

Variants:
- adaptive_rmt: γ ∈ [2, 10] mapped from signal entropy
- adaptive_rmt + dd_target: combined with the Phase 44 wrapper
...
from afp.portfolio.adaptive_rmt_allocator import build_adaptive_rmt_target_portfolio
...
def _ada(d, p, ve, c, k, sec):
    return build_adaptive_rmt_target_portfolio(d, p, ve, c, k=k, sector_map=sec)
...
def main():
    ...
    results = []
    results.append(_run("adaptive_rmt", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _ada, shared_vol))
    results.append(_run("adaptive_rmt+dd_target", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _ada, shared_vol, dd_cfg=dd_cfg))

    out_dir = Path("artifacts/experiments/phys_v7")
