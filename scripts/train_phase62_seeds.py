"""Phase 66: train 5 clean NEW seeds of Phase 62 model.

Pre-declared seeds: 100, 101, 102, 103, 104 (never evaluated before).
Each trains LambdaRank with cross-disciplinary features on tier1
train pool. Predictions saved as `lambdarank_v3_cross_features_sNNN.parquet`.

The ensemble is the equal-weight cross-sectional rank average,
constructed in run_phys_v66_eval.py — this script just produces the
...
def main():
    ...
    print("computing cross features...", flush=True)
    ...
    for seed in SEEDS:
        ...
        preds["model_id"] = f"lambdarank_v3_cross_features_s{seed}"
        ...
        out = Path(f"artifacts/predictions/lambdarank_v3_cross_features_s{seed}.parquet")
