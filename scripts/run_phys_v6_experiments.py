"""Phase 48 experiment: James-Stein signal shrinkage.

Variants:
- js_rmt_minvar: JS-shrunken signal + RMT min-var
- js_rmt_minvar + dd_target: with the Phase 44 wrapper on top
...
from afp.portfolio.js_allocator import build_js_rmt_target_portfolio
...
def _js(d, p, ve, c, k, sec):
    return build_js_rmt_target_portfolio(d, p, ve, c, k=k, sector_map=sec)
...
def main():
    ...
    results = []
    results.append(_run("js_rmt_minvar", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _js, shared_vol))
    results.append(_run("js_rmt_minvar+dd_target", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _js, shared_vol, dd_cfg=dd_cfg))

    out_dir = Path("artifacts/experiments/phys_v6")
