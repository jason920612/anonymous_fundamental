"""Phase 62 evaluation: backtest cross-feature-trained LambdaRank predictions
through Phase 54 stack (rmt_barbell + dd_target). Compare to original v3 model.
"""
...
def main():
    ...
    # New model trained with cross-disciplinary features
    preds_cross = pd.read_parquet("artifacts/predictions/lambdarank_v3_cross_features.parquet")
    results = [_run("rmt_barbell+dd_with_cross_features",
                    preds_cross, prices, benchmarks, calendar,
                    bt_cfg, pcfg, rcfg, k, shared_vol, dd_cfg)]

    out_dir = Path("artifacts/experiments/phys_v20_crossfeat")
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "metrics.json").write_text(json.dumps(results, indent=2))
    pd.DataFrame(results).to_csv(out_dir / "metrics.csv", index=False)

    print("\n=== Phase 62 cross-feature LambdaRank vs Phase 54 baseline ===", flush=True)
    print(f"  Phase 54 (s42 baseline):  Sharpe=1.079  MDD=-0.286  Calmar=0.875", flush=True)
    for r in results:
        print(f"{r['variant']:38s}  Sharpe={r['sharpe']:.3f}  "
