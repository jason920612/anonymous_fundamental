"""Phase 54 experiment: RMT-weighted barbell.

Combines Phase 38 (RMT min-var inside slices) with Phase 53 (Taleb
80/20 barbell). Pre-declared parameters.

Variants:
- rmt_barbell alone
- rmt_barbell + dd_target
"""
...
def main():
    ...
    results = []
    results.append(_run("rmt_barbell", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _rb, shared_vol))
    results.append(_run("rmt_barbell+dd_target", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _rb, shared_vol, dd_cfg=dd_cfg))

    out_dir = Path("artifacts/experiments/phys_v12")
