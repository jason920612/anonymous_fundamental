"""Phase 59 experiment: Michaelis-Menten signal saturation.

Apply M-M (1913 biochemistry) to the candidate signal vector before
the Phase 54 RMT-barbell allocator + DD-target wrapper. Single eval
across tier1 (s42) + tier2 + tier3 for cross-universe robustness in
one shot.
"""
...
from afp.portfolio.mm_allocator import build_mm_rmt_barbell_target_portfolio
...
def _mm(d, p, ve, c, k, sec):
    return build_mm_rmt_barbell_target_portfolio(d, p, ve, c, k=k, sector_map=sec)
...
def _run(name, predictions, prices, benchmarks, calendar, bt_cfg, pcfg, rcfg, k,
    ...
    result = run_backtest(predictions, prices, calendar, bt_cfg, pcfg, rcfg,
                          k=k, sector_map=None, custom_allocator=_mm,
...
def main():
    ...
    out_dir = Path("artifacts/experiments/phys_v17_mm")
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "metrics.json").write_text(json.dumps(results, indent=2))
    pd.DataFrame(results).to_csv(out_dir / "metrics.csv", index=False)

    print("\n=== M-M SATURATION × Phase 54 stack across 3 universes ===", flush=True)
    for r in results:
