"""Phase 64: train LambdaRank with cross-disciplinary MARKET + PER-STOCK features.

Builds on Phase 62 (market features) by adding 6 more PER-STOCK
features (Hurst, skew, kurtosis, Lévy-z, Lyapunov, DFA-α) computed
from each candidate's own recent return series.

Total added: 12 features + 12 missing flags = 24 columns. Plus the
6 market-level cross features and their missing flags (Phase 62).
"""
...
def main():
    ...
    # ------- Market-level cross features (Phase 62) -------
    print("computing market-level cross features...", flush=True)
    ...
    print(f"  market cross features: {time.time()-t0:.1f}s", flush=True)
    ...
    # ------- Per-stock cross features (Phase 64) -------
    print("computing per-stock cross features...", flush=True)
    t0 = time.time()
    per_train = compute_per_stock_features(train, prices)
    per_val = compute_per_stock_features(val, prices)
    per_test = compute_per_stock_features(test, prices)
    print(f"  per-stock cross features: {time.time()-t0:.1f}s", flush=True)
    train = train.merge(per_train, on="sample_id", how="left")
    val = val.merge(per_val, on="sample_id", how="left")
    test = test.merge(per_test, on="sample_id", how="left")
    ...
    def _ds(samples_df, price_df, facts_to_use):
        ...
        # Per-stock cross features
        extra_s = sub_c[PER_STOCK_FEATURE_IDS].to_numpy(dtype=np.float64)
        extra_sm = sub_c[[f"{f}_missing" for f in PER_STOCK_FEATURE_IDS]].to_numpy(dtype=np.float64)
        base.X = np.concatenate([base.X, extra_p, extra_pm, extra_c, extra_cm,
                                  extra_s, extra_sm], axis=1)
    ...
    print(f"feature shape with full cross features: {train_ds.X.shape}", flush=True)
    ...
    print("training LambdaRank with market + per-stock cross features...", flush=True)
    ...
    preds["model_id"] = "lambdarank_v3_full_cross"
    preds["model_type"] = "lambdarank_full_cross"
    preds["prediction_created_at"] = datetime.now(tz=timezone.utc).isoformat()
    joined = attach_prediction_context(preds, samples)
    out = Path("artifacts/predictions/lambdarank_v3_full_cross.parquet")
    ...
    imp = imp.sort_values("gain", ascending=False).head(40)
    print("\ntop-40 feature gains:", flush=True)
