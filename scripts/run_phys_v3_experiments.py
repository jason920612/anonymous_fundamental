"""Phase 44 experiment: drawdown-targeting wrapper.

Apply the DD-target wrapper on top of:
- baseline (inverse-vol)
- rmt_minvar (current Sharpe winner)

And one combined stack:
- rmt_minvar + DD-target + vol-target (full risk-control stack)

3 variants, ~15-18 minutes wall clock. Single-pass evaluation;
parameters pre-declared in docs/phase44.
"""
...
from afp.portfolio.vol_target import VolTargetConfig
...
def _run(name, predictions, prices, benchmarks, calendar, bt_cfg, pcfg, rcfg, k,
         custom_allocator, vol_estimator, dd_cfg=None, vol_target_cfg=None):
    print(f"\n=== {name} ===", flush=True)
    t0 = time.time()
...
def main():
    ...
    vol_cfg = VolTargetConfig(enabled=True, target_annual_vol=0.20,
                              lookback_days=60, min_scale=0.30, max_scale=1.10,
                              blend=0.7)

    results = []
    results.append(_run("baseline+dd_target", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, None, shared_vol, dd_cfg=dd_cfg))
    results.append(_run("rmt_minvar+dd_target", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _rmt, shared_vol, dd_cfg=dd_cfg))
    results.append(_run("rmt_minvar+dd_target+vol_target", predictions, prices, benchmarks,
                        calendar, bt_cfg, pcfg, rcfg, k, _rmt, shared_vol,
                        dd_cfg=dd_cfg, vol_target_cfg=vol_cfg))

    out_dir = Path("artifacts/experiments/phys_v3")
