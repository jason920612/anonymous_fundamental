"""Phase 47 experiment: eigenportfolio market-mode signal subtraction.

Variants:
- eigenport_minvar (alone): does removing market-direction signal help?
- eigenport_minvar + dd_target: combine with the Phase 44 winner wrapper.

2 variants, ~12 minutes wall clock.
"""
...
from afp.portfolio.eigenport_allocator import build_eigenport_target_portfolio
...
def _eigen(d, p, ve, c, k, sec):
    return build_eigenport_target_portfolio(d, p, ve, c, k=k, sector_map=sec)
...
def main():
    ...
    results = []
    results.append(_run("eigenport_minvar", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _eigen, shared_vol))
    results.append(_run("eigenport_minvar+dd_target", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _eigen, shared_vol, dd_cfg=dd_cfg))

    out_dir = Path("artifacts/experiments/phys_v5")
