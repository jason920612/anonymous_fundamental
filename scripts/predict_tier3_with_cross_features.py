"""Phase 65: Apply Phase 62 LambdaRank-with-cross-features model to tier3.

Re-uses the tier3 sample-building pipeline but augments features
with tier3-computed cross-disciplinary market features. The model
is RE-TRAINED on tier1 train pool with cross features (same as
Phase 62) and applied to tier3 test rows.

Output: artifacts/predictions/lambdarank_tier3_cross.parquet
"""
...
def main():
    ...
    # ---------- tier3 test ----------
    samples_t3 = pd.read_parquet("data/processed/tier3/event_samples.parquet")
    facts_t3 = pd.read_parquet("data/processed/tier3/financial_facts_long.parquet")
    prices_t3 = pd.read_parquet("data/processed/tier3/prices_daily.parquet")
    test_t3 = samples_t3[samples_t3["split"] == "test"].dropna(subset=["target_normalized_signal"])
    ...
    print("computing tier3 cross features...", flush=True)
    pivot_t3 = (prices_t3.assign(date=pd.to_datetime(prices_t3["date"]))
                          .pivot_table(index="date", columns="internal_company_id",
                                       values="adjusted_close", aggfunc="last").ffill())
    cross_t3 = compute_cross_features(pivot_t3)

    train = attach_cross_features_to_samples(train, cross_t1)
    val = attach_cross_features_to_samples(val, cross_t1)
    test_t3 = attach_cross_features_to_samples(test_t3, cross_t3)
    ...
    pf_test, _ = compute_price_features(test_t3, prices_t3, pf_cfg)
    ...
    test_ds, test_rows = _ds(test_t3, pf_test, facts_t3)
    print(f"tier3 X shape with cross features: {test_ds.X.shape}", flush=True)
    ...
    preds["model_id"] = "lambdarank_tier3_cross"
    preds["model_type"] = "lambdarank_tier3_cross"
    preds["prediction_created_at"] = datetime.now(tz=timezone.utc).isoformat()
    joined = attach_prediction_context(preds, samples_t3)
    out = Path("artifacts/predictions/lambdarank_tier3_cross.parquet")
