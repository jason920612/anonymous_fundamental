"""Phase 60+61 wrapper experiment: Gutenberg-Richter + Kuramoto.

Quick test of whether these cross-disciplinary regime signals add
value as exposure wrappers on top of Phase 54. tier1 s42 only (no
need to burn full cross-universe budget if signal turns out noisy).
"""
...
from afp.portfolio.gutenberg_richter import GutenbergRichterConfig
from afp.portfolio.kuramoto import KuramotoConfig, compute_kuramoto_r_series
...
def _run(name, predictions, prices, benchmarks, calendar, bt_cfg, pcfg, rcfg, k,
         vol_estimator, dd_cfg=None, gr_cfg=None, kuramoto_cfg=None, kr_series=None):
    print(f"\n=== {name} ===", flush=True)
    t0 = time.time()
    result = run_backtest(predictions, prices, calendar, bt_cfg, pcfg, rcfg,
                          ...
                          gr_cfg=gr_cfg, kuramoto_cfg=kuramoto_cfg,
                          kuramoto_r_series=kr_series)
...
def main():
    ...
    predictions = pd.read_parquet("artifacts/predictions/lambdarank_v3_p100_s42.parquet")
    ...
    print("computing Kuramoto r series...", flush=True)
    t0 = time.time()
    ...
    r_series = compute_kuramoto_r_series(price_pivot, sigma_window=60)
    print(f"  kuramoto r series: {time.time()-t0:.1f}s ({len(r_series.dropna())} pts)",
          flush=True)

    dd_cfg = DrawdownTargetConfig(enabled=True, lookback_days=252,
                                   dd_trigger=0.10, alpha=1.0,
                                   scale_floor=0.30, blend=0.5)
    gr_cfg = GutenbergRichterConfig(enabled=True, event_sigma=1.0,
                                     sigma_window=60, count_window=60,
                                     history_window=252, boltzmann_kt=2.0,
                                     scale_floor=0.30, blend=0.5)
    ku_cfg = KuramotoConfig(enabled=True, sigma_window=60, history_window=252,
                            boltzmann_kt=2.0, scale_floor=0.30, blend=0.5)

    results = []
    results.append(_run("rmt_barbell+dd (baseline winner)",
                        predictions, prices, benchmarks, calendar, bt_cfg, pcfg, rcfg, k,
                        shared_vol, dd_cfg=dd_cfg))
    results.append(_run("rmt_barbell+dd+gr",
                        predictions, prices, benchmarks, calendar, bt_cfg, pcfg, rcfg, k,
                        shared_vol, dd_cfg=dd_cfg, gr_cfg=gr_cfg))
    results.append(_run("rmt_barbell+dd+kuramoto",
                        predictions, prices, benchmarks, calendar, bt_cfg, pcfg, rcfg, k,
                        shared_vol, dd_cfg=dd_cfg, kuramoto_cfg=ku_cfg, kr_series=r_series))
    results.append(_run("rmt_barbell+dd+gr+kuramoto",
                        predictions, prices, benchmarks, calendar, bt_cfg, pcfg, rcfg, k,
                        shared_vol, dd_cfg=dd_cfg, gr_cfg=gr_cfg,
                        kuramoto_cfg=ku_cfg, kr_series=r_series))

    out_dir = Path("artifacts/experiments/phys_v18_gr_kuramoto")
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "metrics.json").write_text(json.dumps(results, indent=2))
    pd.DataFrame(results).to_csv(out_dir / "metrics.csv", index=False)

    print("\n=== GR + Kuramoto wrapper ablation (tier1 s42) ===", flush=True)
    for r in results:
