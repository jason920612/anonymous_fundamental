"""Phase 53 experiment: antifragile barbell allocator.

Variants:
- barbell (80% safe diversified + 20% top-5 concentrated)
- barbell + dd_target
"""
...
from afp.portfolio.barbell_allocator import build_barbell_target_portfolio
...
def _bb(d, p, ve, c, k, sec):
    return build_barbell_target_portfolio(d, p, ve, c, k=k, sector_map=sec)
...
def main():
    ...
    results = []
    results.append(_run("barbell", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _bb, shared_vol))
    results.append(_run("barbell+dd_target", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _bb, shared_vol, dd_cfg=dd_cfg))

    out_dir = Path("artifacts/experiments/phys_v11")
