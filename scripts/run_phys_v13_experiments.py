"""Phase 55 experiment: vol-aware adaptive DD trigger.

Replaces the static dd_trigger=0.10 with `z·σ·sqrt(252/12)` —
the 1-month VaR-style threshold, where z=2.0 (2σ event) and σ is
the rolling strategy daily-vol. Clipped to [0.05, 0.20] for safety.

Variants:
- rmt_barbell + dd_target with adaptive_trigger=True (Phase 54 winner + Phase 55 trigger)
"""
...
def main():
    ...
    dd_cfg_adaptive = DrawdownTargetConfig(
        enabled=True, lookback_days=252, dd_trigger=0.10, alpha=1.0,
        scale_floor=0.30, blend=0.5,
        adaptive_trigger=True, vol_window_days=60, z_score=2.0,
        horizon_months=1.0,
        adaptive_trigger_min=0.05, adaptive_trigger_max=0.20)

    results = []
    results.append(_run("rmt_barbell+dd_adaptive_trigger",
                        predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _rb, shared_vol, dd_cfg=dd_cfg_adaptive))

    out_dir = Path("artifacts/experiments/phys_v13")
