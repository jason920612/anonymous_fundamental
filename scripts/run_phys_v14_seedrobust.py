"""Phase 56: cross-seed robustness of Phase 54 winner.

Run rmt_barbell + dd_target on TWO additional model seeds (s7, s99)
to verify the Phase 54 win is not seed-specific. The default LambdaRank
seed in production is 42; we evaluate s7 and s99 as independent samples.

Critical question: does the +6% Sharpe / -34% MDD / +37% Calmar
improvement vs baseline survive seed variation?

Note: the s7 and s99 predictions parquets use `p50` (max_position=50)
not p100, so the Sharpe levels will differ from the headline run. We
compare APPLES-TO-APPLES: baseline inverse-vol on the SAME seed vs
rmt_barbell + dd_target on the SAME seed.
"""
...
def main():
    ...
    seeds = [
        ("s7",  "artifacts/predictions/lambdarank_v3_p50_s7.parquet"),
        ("s99", "artifacts/predictions/lambdarank_v3_p50_s99.parquet"),
    ]

    results = []
    for tag, path in seeds:
        preds = pd.read_parquet(path)
        results.append(_run(f"baseline_inv_vol_{tag}", preds, prices, benchmarks, calendar,
                            bt_cfg, pcfg, rcfg, k, None, shared_vol))
        results.append(_run(f"rmt_barbell+dd_{tag}", preds, prices, benchmarks, calendar,
                            bt_cfg, pcfg, rcfg, k, _rb, shared_vol, dd_cfg=dd_cfg))

    out_dir = Path("artifacts/experiments/phys_v14")
    ...
    for r in results:
        print(f"{r['variant']:32s}  Sharpe={r['sharpe']:.3f}  "
