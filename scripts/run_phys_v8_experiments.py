"""Phase 50 experiment: vol-of-vol target.

Variants:
- rmt_minvar + vov_target  (vov alone)
- rmt_minvar + dd_target + vov_target  (full predictive+reactive stack)
...
from afp.portfolio.vov_target import VovTargetConfig
...
def _run(name, predictions, prices, benchmarks, calendar, bt_cfg, pcfg, rcfg, k,
         custom_allocator, vol_estimator, dd_cfg=None, vov_cfg=None):
    print(f"\n=== {name} ===", flush=True)
    t0 = time.time()
...
def main():
    ...
    vov_cfg = VovTargetConfig(enabled=True, sigma_window=20, vov_window=60,
                              boltzmann_kt=2.0, scale_floor=0.30, blend=0.5)

    results = []
    results.append(_run("rmt_minvar+vov_target", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _rmt, shared_vol, vov_cfg=vov_cfg))
    results.append(_run("rmt_minvar+dd+vov", predictions, prices, benchmarks, calendar,
                        bt_cfg, pcfg, rcfg, k, _rmt, shared_vol,
                        dd_cfg=dd_cfg, vov_cfg=vov_cfg))

    out_dir = Path("artifacts/experiments/phys_v8")
